...
import org.aspectj.ajdt.internal.compiler.ast.perscope.ParametersParser;
...
import org.aspectj.ajdt.internal.compiler.ast.perscope.storage.PerscopeDelegator;
...
public class PatternParser {
	...
	public PerClause maybeParsePerClause() {
		...
		if (tok == IToken.EOF)
			return defaultPerscope();
		if (tok.isIdentifier()) {
			String name = tok.getString();
			return parsePerscope();
		}
		return null;
	}
	...
	private PerClause parsePerscope() {
		parseIdentifier();
		eat("(");
		//String type = parseIdentifier();
		//System.out.println("PERSCOPE type = " + type);
		//eat("=");
		String perscopeClassName = parsePerscopeClassName();
		
		Object instance = null;
		try {
			Class<?> forName = Class.forName(perscopeClassName);
			instance = forName.newInstance();
		} catch (Exception e) {
			throw new RuntimeException("failed to instantiate the perscope class", e);
		} 
		if (instance instanceof StaticPerscopeParse) {
		//if ("aspectWeavingClass".equals(type)) {
			return parseStaticPerscope(instance);
		}
		else {
			return parseDynamicPerscope(instance);
		}
	}
	
	private PerClause parseStaticPerscope(Object instance) {
		PerClause perClause = null;
		StaticPerscopeParse perClauseAspectWeaving = (StaticPerscopeParse) instance;
		ParametersParser perClauseAspectParser = perClauseAspectWeaving.getPerClauseAspectParser();
		perClauseAspectParser.setPatternParser(this);
		eat(",");
		perClause = perClauseAspectParser.parsePerClause();
		eat(")");
		//perClause = perClauseAspectWeaving.getPerClause(entry);
		perClause.setStaticPerscope(perClauseAspectWeaving);
	
		return perClause;
	}
	
	private PerClause parseDynamicPerscope(Object instance) {
		eat(",");
		Pointcut entry = parsePointcut();
		eat(")");
		PerClause perClause = new PerObject(entry, false);
		StaticPerscope perScopeRelated = new PerscopeDelegator();
		perClause.setStaticPerscope(perScopeRelated);
		perScopeRelated.setImplementationName(instance.getClass().getName());
		return perClause;
	}
	...
	private PerClause parsePerObject(boolean isThis) {
		parseIdentifier();
		eat("(");
		String aspectsStorageClassName = parsePerscopeClassName();
		eat(",");
		Pointcut entry = parsePointcut();
		eat(")");
		return new PerObject(entry, isThis, aspectsStorageClassName);
	}
	// VICTOR
	private String parsePerscopeClassName() {
